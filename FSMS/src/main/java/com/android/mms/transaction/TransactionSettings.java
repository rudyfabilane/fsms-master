...
//            Log.e(TAG, "Apn is not found in Database!");
            if (com.rojangames.freetextph.mmssms.Transaction.settings == null) {
                com.rojangames.freetextph.mmssms.Transaction.settings = Utils.getDefaultSendSettings(context);
            }

            mServiceCenter = NetworkUtils.trimV4AddrZeros(com.rojangames.freetextph.mmssms.Transaction.settings.getMmsc());
            mProxyAddress = NetworkUtils.trimV4AddrZeros(com.rojangames.freetextph.mmssms.Transaction.settings.getProxy());

            // Set up the agent, profile url and tag name to be used in the mms request if they are attached in settings
            String agent = com.rojangames.freetextph.mmssms.Transaction.settings.getAgent();
            if (agent != null && !agent.trim().equals("")) {
                MmsConfig.setUserAgent(agent);
                if (LOCAL_LOGV) Log.v(TAG, "set user agent");
            }

            String uaProfUrl = com.rojangames.freetextph.mmssms.Transaction.settings.getUserProfileUrl();
            ...
            String uaProfTagName = com.rojangames.freetextph.mmssms.Transaction.settings.getUaProfTagName();
            ...
            if (isProxySet()) {
                try {
                    mProxyPort = Integer.parseInt(com.rojangames.freetextph.mmssms.Transaction.settings.getPort());
                } catch (NumberFormatException e) {
                    Log.e(TAG, "could not get proxy: " + com.rojangames.freetextph.mmssms.Transaction.settings.getPort(), e);
                }
            }
//        }
